...
import static com.aaa.sportclub.data.Contacts.memberEntry.CONTENT_URI;
import static com.aaa.sportclub.data.Contacts.memberEntry.GENDER_FEMALE;
import static com.aaa.sportclub.data.Contacts.memberEntry.GENDER_MALE;
import static com.aaa.sportclub.data.Contacts.memberEntry.GENDER_UNKNOWN;
...
public class AddMemberActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private void saveMember() {
        ...
        if (TextUtils.isEmpty(firstName)) {
            ...
            Toast.makeText(this, "Input the sport group", Toast.LENGTH_LONG).show();
            return;
    ...
    public void onLoadFinished(@NonNull Loader<Cursor> loader, Cursor cursor) {
        if (cursor.moveToFirst()) {
            int firtNameColumnIndex = cursor.getColumnIndex(COLUMN_FIRST_NAME);
            int lastNameColumnIndex = cursor.getColumnIndex(COLUMN_LAST_NAME);
            int genderColumnIndex = cursor.getColumnIndex(COLUMN_GENDER);
            int sportColumnIndex = cursor.getColumnIndex(COLUMN_SPORT);

            String firstName = cursor.getString(firtNameColumnIndex);
    ...
    private void deleteMember() {
        if (currentMemberUri != null) {
            ...
            if (rowsDeleted == 0) {
                Toast.makeText(this, "Delete of data from the failed", Toast.LENGTH_LONG).show();
            } else {
